...
import { ChevronLeft, ChevronRight, Clock, Star, ChefHat, Share2 } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { recipes } from "@/data/recipes";

const heroRecipes = recipes.slice(0, 7);

const HeroSection = () => {
  const navigate = useNavigate();
  const [activeIndex, setActiveIndex] = useState(0);
  const activeRecipe = heroRecipes[activeIndex];
  ...
  // Arc positions for orbital thumbnails
  const orbitPositions = useMemo(() => {
    const count = heroRecipes.length;
  ...
  // 3 recipe cards to show below
  const cardRecipes = useMemo(() => {
    const cards = [];
    for (let i = 1; i <= 3; i++) {
      cards.push(heroRecipes[(activeIndex + i) % heroRecipes.length]);
    }
    return cards;
  }, [activeIndex]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-4 items-center min-h-[520px]">
          ...
          <motion.div className="lg:col-span-5 space-y-6 z-10" style={{ y: contentY }}>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  const url = `${window.location.origin}/recipe/${activeRecipe.id}`;
                  if (navigator.share) {
                    navigator.share({ title: activeRecipe.title, text: activeRecipe.description, url });
                  } else {
          ...
          <motion.div className="lg:col-span-7 relative flex flex-col items-center" style={{ y: orbitY }}>
            <div className="relative w-full" style={{ paddingBottom: "85%" }}>
              ...
              <svg
                ...
                <circle
                  ...
                  opacity={i === activeIndex ? 0 : 0.3} />

                )}
              </svg>

              {/* Orbiting thumbnails — bigger size */}
              ...
              {/* Active hero image with callouts */}
              <AnimatePresence mode="wait">
                <motion.div
                  key={activeRecipe.id}
                  ...
                  {/* Callout: Prep Time — top left */}
                  <motion.div
                    initial={{ opacity: 0, scale: 0.5, x: 20 }}
                    animate={{ opacity: 1, scale: 1, x: 0 }}
                    exit={{ opacity: 0, scale: 0.5 }}
                    transition={{ delay: 0.3, type: "spring", stiffness: 200, damping: 15 }}
                    className="absolute -top-2 -left-8 bg-card border border-border rounded-xl px-3 py-2 shadow-card flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-semibold text-foreground">{activeRecipe.prepTime}</p>
                    </div>
                  </motion.div>

                  {/* Callout: Rating — top right */}
                  <motion.div
                    initial={{ opacity: 0, scale: 0.5, x: -20 }}
                    animate={{ opacity: 1, scale: 1, x: 0 }}
                    exit={{ opacity: 0, scale: 0.5 }}
                    transition={{ delay: 0.4, type: "spring", stiffness: 200, damping: 15 }}
                    className="absolute -top-2 -right-6 bg-card border border-border rounded-xl px-3 py-2 shadow-card flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-semibold text-foreground">{activeRecipe.rating}</p>
                    </div>
                  </motion.div>

                  {/* Callout: Difficulty — bottom right */}
                  <motion.div
                    initial={{ opacity: 0, scale: 0.5, y: -20 }}
                    animate={{ opacity: 1, scale: 1, y: 0 }}
                    exit={{ opacity: 0, scale: 0.5 }}
                    transition={{ delay: 0.5, type: "spring", stiffness: 200, damping: 15 }}
                    className="absolute bottom-4 -right-10 bg-card border border-border rounded-xl px-3 py-2 shadow-card flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-semibold text-foreground">{activeRecipe.difficulty}</p>
                    </div>
                  </motion.div>
                </motion.div>
              </AnimatePresence>
            ...
            {/* Navigation arrows under the picture */}
            <div className="flex items-center -mt-8 z-30 py-0 gap-[113px]">
              <motion.button
                onClick={prev}
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
                className="w-10 h-10 rounded-full border border-border bg-background flex items-center justify-center text-foreground hover:border-primary transition-colors">
